...
from modAL.diversity import diversity_sampling
...
preset_diversity_img = update_wrapper(
    partial(diversity_sampling, transform=(lambda x: x[0]), n_instances=BATCH_SIZE),
    diversity_sampling
)

preset_diversity_txt = update_wrapper(
    partial(diversity_sampling, transform=(lambda x: x[1]), n_instances=BATCH_SIZE),
    diversity_sampling
)

preset_diversity_concat = update_wrapper(
    partial(diversity_sampling, transform=(lambda x: np.concatenate(x, axis=1)), n_instances=BATCH_SIZE),
    diversity_sampling
)

query_dict = {
    'diversity_img': preset_diversity_img,
    'diversity_txt': preset_diversity_txt,
    'diversity_concat': preset_diversity_concat
}
