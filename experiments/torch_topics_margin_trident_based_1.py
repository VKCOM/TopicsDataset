...
from experiments.models.torch_topics_decorator import TopicsDecorator, TridentDecorator, TridentAsNormDecorator

from modAL import KerasActiveLearner
from modAL.qbc_dropout import bald_trident_based_sampling
...
query_dict = {
    'margin_trident_based': preset_margin
}

for i in range(1, 2):
    for retrain_epoch in [1, 5, 10]:
        np.random.seed(i)
        ...
        model = NormModelTrident(drop=0.5, d=128)
        optimizer = optim.Adam(model.parameters(), lr=1e-3, weight_decay=0.0005)

        decorated_model = TridentAsNormDecorator(model, optimizer)
        decorated_model.fit(
            X=x_init_train,
            y=y_init_train,
            epochs=INIT_EPOCHS,
        ...
        for query_name in query_dict:
            ...
            decorated_model = deepcopy(decorated_model)
            ...
            experiment = Experiment(
                ...
                random_seed=i,
                pool_size=POOL_SIZE,
                name='torch_topics_d128_' + query_name + '_e' + str(retrain_epoch) + '_i2000_b20_q100_sf512_' + str(i),
                bootstrap=False,
                epochs=retrain_epoch
            )

            experiment.run()
            experiment.save_state('statistic/topics/torch/d128/' + query_name + '_e' + str(retrain_epoch) + '_i2000_b20_q100_sf512_' + str(i))
