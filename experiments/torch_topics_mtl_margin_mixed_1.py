...
from experiments.models.topics_torch_models import NormModelTridentBN, TridentMTL
from experiments.datasets.topics_ds import get_unpacked_data
from experiments.al_experiment import Experiment
from experiments.models.torch_topics_decorator import TridentAsNormDecorator, TridentDecorator, TridentMTLDecorator

from modAL import KerasActiveLearner
from modAL.uncertainty import margin_trident_sampling, margin_mixed_sampling
...
preset_margin_mixed= update_wrapper(
    partial(margin_mixed_sampling, n_instances=BATCH_SIZE),
    margin_mixed_sampling
)
query_dict = {
    'margin_mixed': preset_margin_mixed
}
...
for i in range(1, 2):
    ...
    decorated_model = TridentMTLDecorator(model, optimizer)
    decorated_model.fit(
        X=x_init_train,
        y=y_init_train,
        epochs=INIT_EPOCHS,
    ...
    for query_name in query_dict:
        ...
        experiment = Experiment(
            ...
            random_seed=i,
            pool_size=POOL_SIZE,
            name='torch_topics_mtl_init_bn_' + query_name + '_i2000_b20_q400_' + str(i),
            intermediate_state_saving=True,
            intermediate_state_filename='statistic/topics/torch_mtl_init_bn/' + query_name + '_i2000_b20_q400_inter_' + str(i),
        ...
        experiment.save_state(
            'statistic/topics/torch_mtl_init_bn/' + query_name + '_i2000_b20_q400_' + str(i))
