...
for i in range(1, 2):
    for sud_param in sud_params:
        ...
        preset_density_transform = update_wrapper(partial(
            ...
            sparse=True,
            top_uncertain_samples=top_uncertain_samples,
            top_sud_samples=top_sud_samples,
        ), sud_margin_logit)
        ...
        experiment = Experiment(
            ...
            random_seed=i,
            pool_size=POOL_SIZE,
            name='torch_topics_sud_top' + str(sud_param) + '_trivial_encode_ad64_margin_logit_sparse_i2000_b20_q200_indices_' + str(i)
        )

        experiment.run()
        experiment.save_state(
            'statistic/topics/torch_bn/sud_top' + str(sud_param) + '_trivial_encode_ad64_margin_logit_sparse_i2000_b20_q200_indices_' + str(i))

        print(top_uncertain_samples)
        print(top_sud_samples)